...
//            databaseStore.updateUser(user);
    ...
    @Override
    public WFCMessage.User getUserInfoByNameAndPwd(String name, String pwdMd5) {
        String userId = databaseStore.getUserIdByNameAndPwd(name,pwdMd5);
        return getUserInfo(userId);
    }
    ...
    @Override
    public ErrorCode updateUserPwd(String userId, String password) {
        databaseStore.updateUserPassword(userId, password);
        return ErrorCode.ERROR_CODE_SUCCESS;
    }
    ...
    public boolean checkPassword(String userId, String password) {
        ...
        try {
            connection = DBUtil.getConnection();
            String sql = "select `_uid`, `_passwd_md5` from t_user where `_uid` = ?";
            ...
            if (rs.next()) {
                ...
                try {
                    ...
                    String passwdMd5 = base64en.encode(md5.digest(password.getBytes("utf-8")));
                    if (passwdMd5.equals(pwd_md5)) {
                        LOG.info("login success userName={}, userId={}", userId, uid);
                        return true;
                    } else {
                        LOG.info("login failure incorrect password, userId={}",  uid);
                        return false;
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    Utility.printExecption(LOG, e);
                    LOG.info("login failure execption password, userId={}", userId);
                    return false;
                }
            } else {
                LOG.info("login failure user not exist, userName={}", userId);
                return false;
            }
        } catch (SQLException e) {
            e.printStackTrace();
            Utility.printExecption(LOG, e);
            LOG.info("login failure db execption, userName={}", userId);
            return false;
        } finally {
            DBUtil.closeDB(connection, statement, rs);
        }
