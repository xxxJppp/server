...
//    }
    ...
    public static IMResult<ErrorCode> checkPwdLogin(String name,String password) throws Exception {
        String path = APIPath.Check_User_Pwd;
        InputGetUserInfo getUserInfo = new InputGetUserInfo();
        getUserInfo.setName(name);
        getUserInfo.setPassword(password);
        return AdminHttpUtils.httpJsonPost(path, getUserInfo, ErrorCode.class);
    }

    public static IMResult<ErrorCode> updatePwd(String userId, String oldpassword, String password) throws Exception {
        String path = APIPath.Update_User_Pwd;
        InputGetUserInfo getUserInfo = new InputGetUserInfo();
        getUserInfo.setUserId(userId);
        getUserInfo.setPassword(password);
        getUserInfo.setOldpassword(oldpassword);
        return AdminHttpUtils.httpJsonPost(path, getUserInfo, ErrorCode.class);
    }
}
